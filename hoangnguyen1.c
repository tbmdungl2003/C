...
{   
     printf("Danh sach thi sau lan thi thu hai:\n");
     printf("%-8s%-10s%-20s%-8s%-8s%-8s%-8s","ID","Nation","Name","R1","R2","R3","FR");
	 for(int i=0;i<n;i++)
	 {
...
{   
     printf("Danh sach thi sau lan thi thu ba:\n");
     printf("%-8s%-10s%-20s%-8s%-8s%-8s%-8s","ID","Nation","Name","R1","R2","R3","FR");
	 for(int i=0;i<n;i++)
	 {
	 	printf("\n%-8d%-10s%-20s%-8.2f%-8.2f%-8.2f%-5.2f",vdv[i].id,vdv[i].nation,vdv[i].name,vdv[i].a[0]  ,vdv[i].a[1],vdv[i].a[2],  max(  vdv[i].a[0],vdv[i].a[1],vdv[i].a[2]));
	 }
}

void timkiemqg (vandong_vien vdv[],int n)
{
	printf("Nhap ten quoc gia ban muon tim kiem: ");
	char ten[30];
	getchar();
	gets(ten);
